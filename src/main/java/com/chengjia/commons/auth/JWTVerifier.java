package com.chengjia.commons.auth;
...
import com.chengjia.commons.auth.exception.JWTAudienceException;
import com.chengjia.commons.auth.exception.JWTExpiredException;
import com.chengjia.commons.auth.exception.JWTIssuerException;
import com.chengjia.commons.auth.exception.JWTVerifyException;
...
public class JWTVerifier {
	 ...
	 *             when expiration, issuer or audience are invalid
	 * @throws IllegalStateException
	 *             when token's structure is invalid
	 */
	...
	void verifyAudience(JsonNode jwtClaims) throws JWTAudienceException {
		...
		if (audNode.isArray()) {
			for (JsonNode jsonNode : audNode) {
				if (audience.equals(jsonNode.getTextValue()))
					return;
			}
		} else if (audNode.isTextual()) {
			if (audience.equals(audNode.getTextValue()))
				return;
		}
