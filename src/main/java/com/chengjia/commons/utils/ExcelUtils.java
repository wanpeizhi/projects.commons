...
public class ExcelUtils {
	 ...
	 */
	private HSSFSheet createHSSFSheet(HSSFWorkbook hssfWorkbook, String sheetName) {
	...
	private HSSFSheet getHSSFSheet(HSSFWorkbook hssfWorkbook, String sheetName) {
		LOGGER.info("【获得 sheet】sheetName ： " + sheetName);
		return hssfWorkbook.getSheet(sheetName);
	}

	/**
	 * 根据sheet index获取sheet
	 * 
	 * @param hssfWorkbook
	 * @param sheetIndex
	 * @return
	 */
	private HSSFSheet getHSSFSheet(HSSFWorkbook hssfWorkbook, int sheetIndex) {
		LOGGER.info("【创获得sheet】sheetIndex ： " + sheetIndex);
	...
	public static void createExcel2FilePath(String sheetName, String title, String filePath, String[] headers,
		...
		ExcelUtils poiExcelUtil = new ExcelUtils();
		// 1.创建 Workbook
		HSSFWorkbook hssfWorkbook = poiExcelUtil.createHSSFWorkbook();
		// 2.创建 Sheet
		HSSFSheet hssfSheet = poiExcelUtil.createHSSFSheet(hssfWorkbook, sheetName);
		// 3.写入 head
	...
	private void writeExportDate(HSSFWorkbook hssfWorkbook, HSSFRow row, int cellIndex) {
		...
		cellValueHandler(cell4ExportDateValue, DateUtils.getCurDatetimeString());
